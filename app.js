...
function renderParticipant() {
    ...
    for (const rodada in grupos) {
        ...
        // Jogos MM oitavas+: cada jogo tem controle individual → trata como rodada "virtual" por jogo
        if (rnd.eliminatorio && (rnd.faseIndex || 0) >= 1) {
            // Cada jogo individual será classificado dentro da rodada
            // A rodada como agrupador aparece, mas o status é por jogo
            const todosEncerrados = grupos[rodada].every(item =>
                item.game.resultA !== undefined && item.game.resultB !== undefined
            );
            if (todosEncerrados) rodadasEncerradas.push(rodada);
            else rodadasAndamento.push(rodada); // mostra como "em andamento" genérico para aparecer
        } else {
            const status = getRoundStatus(rodada, rounds, grupos);
    ...
    rodadasEncerradas.forEach(rodada => { h += htmlRodadaEncerrada(rodada, grupos, allGuesses, participants, uid); });
    rodadasFechadas.forEach(rodada   => { h += htmlRodadaFechada(rodada, grupos, myGuesses, uid); });
...
function htmlRodadaAndamento(rodada, grupos, allGuesses, participants, uid, rounds) {
    ...
    let jogosParaExibir = grupos[rodada];
    if (isElimIndiv) {
        // Mostra jogos em andamento ou encerrados (não fechados/abertos)
        jogosParaExibir = grupos[rodada].filter(item =>
            item.game.iniciada || (item.game.resultA !== undefined)
        );
        if (jogosParaExibir.length === 0) return "";
    }
...
// ── RODADA ENCERRADA ───────────────────────────────────────────────────────
function htmlRodadaEncerrada(rodada, grupos, allGuesses, participants, uid) {
    const idCollapse = "round_" + rodada.replace(/\s+/g, '_');
    let jogosHtml = "";
    grupos[rodada].forEach(item => { jogosHtml += htmlJogoTabela(item, allGuesses, participants, uid); });
...
// ── RODADA FECHADA ─────────────────────────────────────────────────────────
function htmlRodadaFechada(rodada, grupos, myGuesses, uid) {
    const idCollapse = "round_" + rodada.replace(/\s+/g, '_');
    let jogosHtml = "";
