    ...
    jogos.forEach(item => {
        ...
        jogosHtml += `
            <div style="padding:10px 0;border-bottom:1px solid #eee">
                ...
                <div style="margin-bottom:4px;display:flex;align-items:center;gap:6px">
                    <label style="font-size:0.8rem;color:#555;min-width:110px">Mensagem</label>
                    <input id="gmsg_${item.id}" value="${g.mensagem || ''}" placeholder="Opcional" style="flex:1;font-size:0.8rem;padding:4px 6px">
                    <button class="btn-sm" style="background:#607d8b;color:#fff;font-size:0.78rem;white-space:nowrap" onclick="saveMensagemJogo('${item.id}')">Salvar</button>
                </div>` : ""}
...
function renderParticipant() {
    ...
    if (_abaAtiva === "desempenho") {
        contentEl.innerHTML = htmlAbaDesempenho(grupos, allGuesses, participants, uid, gamesSnap, ranking);
        return;
    }
...
function htmlAbaDesempenho(grupos, allGuesses, participants, uid, gamesSnap, ranking) {
    ...
    const ptsPorTipo   = { PE:25, GV:18, AE:15, GP:12, VP:10 };
    let totalPts = 0;
    let totalAcertos = 0;
    ...
    const aproveitamento = totalJogosComResult > 0 ? Math.round(totalAcertos / totalJogosComResult * 100) : 0;
    ...
    // ── Resumo ────────────────────────────────────────────────────────────
    const tiposOrdem = ["PE","GV","AE","GP","VP"];
    const maxTipo = Math.max(...tiposOrdem.map(t => totalPorTipo[t]||0), 1);
    let tiposHtml = tiposOrdem.map(t => {
        const n = totalPorTipo[t] || 0;
        const cores = { PE:"#085041", GV:"#0C447C", AE:"#3C3489", GP:"#633806", VP:"#444441" };
        const bgs   = { PE:"#E1F5EE", GV:"#E6F1FB", AE:"#EEEDFE", GP:"#FAEEDA", VP:"#F1EFE8" };
        ...
        return `
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
                ...
                <span style="font-size:0.82rem;color:#333;min-width:60px">${n} × ${ptsPorTipo[t]}pts</span>
            </div>`;
    }).join("");

    const rodadasOrdem = Object.keys(ptsPorRodada);
    let rodadasHtml = rodadasOrdem.map(r => `
        <div style="flex:1;min-width:0;background:#f1f8f1;border-radius:7px;padding:8px;text-align:center">
            <div style="font-size:0.7rem;color:#888;margin-bottom:2px">${r.replace("Rodada ","R").replace(" de final","")}</div>
            <div style="font-size:1.1rem;font-weight:700;color:#2e7d32">${ptsPorRodada[r]}</div>
            <div style="font-size:0.7rem;color:#888">${acertosPorRodada[r]} acertos</div>
        </div>`).join("");

    const resumoHtml = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
            <div style="background:#f1f8f1;border-radius:8px;padding:10px 12px">
                <div style="font-size:0.72rem;color:#888;margin-bottom:2px">Total de pontos</div>
                <div style="font-size:1.4rem;font-weight:700;color:#2e7d32">${totalPts}</div>
                <div style="font-size:0.7rem;color:#aaa">${posicao}º lugar</div>
            </div>
            <div style="background:#f1f8f1;border-radius:8px;padding:10px 12px">
                <div style="font-size:0.72rem;color:#888;margin-bottom:2px">Aproveitamento</div>
                <div style="font-size:1.4rem;font-weight:700;color:#2e7d32">${aproveitamento}%</div>
                <div style="font-size:0.7rem;color:#aaa">${totalAcertos} de ${totalJogosComResult}</div>
            </div>
            <div style="background:#f1f8f1;border-radius:8px;padding:10px 12px">
                <div style="font-size:0.72rem;color:#888;margin-bottom:2px">Placares exatos</div>
                <div style="font-size:1.4rem;font-weight:700;color:#2e7d32">${totalPorTipo.PE}</div>
                <div style="font-size:0.7rem;color:#aaa">${totalPorTipo.PE * 25} pts</div>
            </div>
            <div style="background:#f1f8f1;border-radius:8px;padding:10px 12px">
                <div style="font-size:0.72rem;color:#888;margin-bottom:2px">Melhor rodada</div>
                <div style="font-size:1rem;font-weight:700;color:#2e7d32">${melhorRodada.replace("Rodada ","R").replace(" de final","")}</div>
                <div style="font-size:0.7rem;color:#aaa">${melhorPts > 0 ? melhorPts + " pts" : "—"}</div>
            </div>
        </div>
        <div style="font-size:0.72rem;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px">Tipos de acerto</div>
        ${tiposHtml}
        <div style="font-size:0.72rem;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.04em;margin:10px 0 6px">Por rodada</div>
    ...
    Object.keys(jogosPorRodada).forEach(rodada => {
        ...
        jogos.forEach(({ id, game: g }) => {
            const guess = allGuesses[id] && allGuesses[id][pidSel];
            const temResult = g.resultA !== undefined && g.resultB !== undefined;
            let badgeKey, pts = "", palpiteStr = "—";
            if (!guess) {
            ...
            linhasJogo += `
                <div style="display:flex;align-items:center;gap:6px;padding:7px 0;border-bottom:0.5px solid #f0f0f0">
                    <div style="flex:1;min-width:0">
                        <div style="font-size:0.78rem;font-weight:600;color:#333;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${flag(g.a)||""}${g.a} × ${flag(g.b)||""}${g.b}</div>
                        ${g.datetime ? `<div style="font-size:0.68rem;color:#aaa">${formatDatetime(g.datetime)}</div>` : ""}
                    </div>
                    <div style="text-align:center;min-width:38px">
                        <div style="font-size:0.72rem;color:#aaa">palpite</div>
                        <div style="font-size:0.8rem;font-weight:600;color:#333">${palpiteStr}</div>
                    </div>
                    <div style="text-align:center;min-width:38px">
                        <div style="font-size:0.72rem;color:#aaa">real</div>
        ...
        histHtml += `
            <div style="margin-bottom:12px">
                <div style="font-size:0.72rem;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px">${rodada}</div>
                ${linhasJogo}
            </div>`;
    });
    ...
    if (mostrarH2H) {
        ...
        let winsA = 0, winsB = 0;
        const tiposA = { PE:0, GV:0, VP:0 };
        const tiposB = { PE:0, GV:0, VP:0 };
        gamesSnap.forEach(c => {
            const g = c.val();
            const gid = c.key;
            ...
            if (rA && tiposA[rA.cod] !== undefined) tiposA[rA.cod]++;
            if (rB && tiposB[rB.cod] !== undefined) tiposB[rB.cod]++;
        });

        const tiposH2HHtml = ["PE","GV","VP"].map(t => {
            const a = tiposA[t], b = tiposB[t], tot = (a+b)||1;
            const bgT = { PE:"#E1F5EE", GV:"#E6F1FB", VP:"#F1EFE8" };
            const corT= { PE:"#085041", GV:"#0C447C", VP:"#444441" };
            return `
                <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
                    ...
                    <div style="flex:1;height:6px;background:#e0e0e0;border-radius:3px;overflow:hidden;display:flex">
                        <div style="height:6px;background:#1D9E75;width:${Math.round(a/tot*100)}%"></div>
                        <div style="height:6px;background:#378ADD;width:${Math.round(b/tot*100)}%"></div>
                    </div>
            ...
            <div style="margin-top:4px">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
                    <div style="text-align:center;flex:1">
                        ...
                        <div style="font-size:0.82rem;font-weight:600;color:#333">${nomeSel}</div>
                    </div>
                    <div style="font-size:0.8rem;color:#aaa;font-weight:600">vs</div>
                    <div style="text-align:center;flex:1">
                        ...
                        <div style="font-size:0.82rem;font-weight:600;color:#333">${nomeB}</div>
                    </div>
                </div>
                <div style="display:flex;justify-content:space-between;font-size:0.82rem;font-weight:600;margin-bottom:3px">
                    <span style="color:#0f6e56">${ptsA} pts</span>
                    <span style="font-size:0.7rem;color:#aaa;align-self:center">pontuação total</span>
                    <span style="color:#185FA5">${ptsB} pts</span>
                </div>
                <div style="height:8px;background:#e0e0e0;border-radius:4px;overflow:hidden;display:flex;margin-bottom:12px">
                    <div style="height:8px;background:#1D9E75;width:${pctA}%"></div>
                    <div style="height:8px;background:#378ADD;width:${pctB}%"></div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
                    <div style="background:#f1f8f1;border-radius:7px;padding:8px;text-align:center">
                        <div style="font-size:0.7rem;color:#888;margin-bottom:2px">Jogos que você venceu</div>
                        <div style="font-size:1.3rem;font-weight:700;color:#0f6e56">${winsA}</div>
                    </div>
                    <div style="background:#e8f4fd;border-radius:7px;padding:8px;text-align:center">
                        <div style="font-size:0.7rem;color:#888;margin-bottom:2px">Jogos que ele venceu</div>
                        <div style="font-size:1.3rem;font-weight:700;color:#185FA5">${winsB}</div>
                    </div>
                </div>
                <div style="font-size:0.72rem;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px">Comparativo por tipo</div>
                ${tiposH2HHtml}
            </div>`;
    } else {
        h2hHtml = `<div style="text-align:center;color:#aaa;font-size:0.85rem;padding:20px 0">Selecione outro participante para ver o H2H</div>`;
    }
    ...
    return `
        <div class="card">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
                ...
                <div style="flex:1">
                    <div style="font-size:0.78rem;color:#888;margin-bottom:2px">Visualizando desempenho de</div>
