    ...
    app.innerHTML = `
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:2px">
            ...
            <div style="position:relative" id="perfil-wrap">
                <button id="btn-perfil" onclick="togglePerfilMenu()" style="display:flex;align-items:center;gap:5px;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);border-radius:20px;padding:5px 10px;cursor:pointer;font-size:12px;font-weight:600;color:inherit">
                    <span id="perfil-nome-btn">${u.name}</span> <span id="perfil-seta">▾</span>
                </button>
                <div id="perfil-menu" style="display:none;position:absolute;top:calc(100% + 6px);right:0;background:var(--branco,#fff);border:0.5px solid #ddd;border-radius:10px;box-shadow:0 4px 16px rgba(0,0,0,0.15);min-width:170px;overflow:hidden;z-index:999">
                    <div onclick="toggleTheme();togglePerfilMenu()" style="display:flex;align-items:center;gap:8px;padding:10px 14px;cursor:pointer;font-size:13px;border-bottom:0.5px solid #f0f0f0" id="dd-tema">
                        <span id="dd-tema-icon">🌙</span><span id="dd-tema-txt">Modo escuro</span>
                    </div>
                    <div onclick="abrirAlterarSenha('${id}')" style="display:flex;align-items:center;gap:8px;padding:10px 14px;cursor:pointer;font-size:13px;border-bottom:0.5px solid #f0f0f0">
        ...
        <div id="modal-senha" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:9998;align-items:center;justify-content:center;padding:20px">
            <div style="background:var(--branco,#fff);border-radius:14px;padding:20px;width:100%;max-width:300px">
                ...
                <div style="display:flex;gap:8px">
                    <button onclick="fecharAlterarSenha()" style="flex:1;padding:9px;border-radius:8px;border:1px solid #ddd;background:#fff;font-size:12px;cursor:pointer">Cancelar</button>
...
function htmlRodadaAberta(rodada, grupos, myGuesses, allGuesses, participants, uid, rounds) {
    ...
    let statusRows = "";
    if (isElimIndiv) {
        // Oitavas+: status por jogo individual
        let headerCols = '<th style="text-align:left;padding:4px">Participante</th>';
        jogosParaExibir.forEach(item => {
            const g = item.game;
            const abbA = abbr(g.a); const abbB = abbr(g.b);
            headerCols += `<th style="padding:4px;font-size:0.72rem;text-align:center">${abbA}<br>x<br>${abbB}</th>`;
        });
        statusRows = `<tr style="background:#e8f5e9">${headerCols}</tr>`;
        Object.keys(participants).forEach(pid => {
            const destaque = pid === uid ? ' style="background:#fffbe6"' : '';
            let cols = `<td style="padding:4px">${participants[pid]}${pid === uid ? " 👈" : ""}</td>`;
            jogosParaExibir.forEach(item => {
                const temPalpite = allGuesses[item.id] && allGuesses[item.id][pid];
                cols += `<td style="text-align:center;padding:4px;font-size:1.1rem">
                    ${temPalpite
                        ? '<span style="color:#27ae60">●</span>'
                        : '<span style="color:#e74c3c">✕</span>'}
                </td>`;
            });
            statusRows += `<tr${destaque}>${cols}</tr>`;
        });
    } else {
        // 16 avos: status por rodada (comportamento atual)
        Object.keys(participants).forEach(pid => {
