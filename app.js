...
// Converte "2026-06-15T15:00" → "Seg, 15/06 · 15h00"
...
function adminPanel() {
    app.innerHTML = `
        ...
        <button id="btn-theme" onclick="toggleTheme()" style="margin-left:8px;padding:5px 12px;font-size:0.8rem;border-radius:20px;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);color:#fff;font-weight:700;cursor:pointer">🌙 Escuro</button>
        ...
        <div class="card">
            ...
            <input id="grupo" placeholder="Grupo (ex: Grupo A) — opcional, deixe vazio para eliminatórias">
...
function htmlRodadaAdmin(rodada, status, jogos) {
    ...
    jogos.forEach(item => {
        ...
        const metaInfo = [
            g.grupo    ? `<span style="background:#e3f2fd;color:#1565c0;padding:1px 8px;border-radius:10px;font-size:0.78rem">${g.grupo}</span>` : "",
    ...
    document.querySelectorAll('[id^="round_"]').forEach(el => {
        ...
        function htmlRodadaAberta(rodada) {
            ...
            grupos[rodada].forEach(item => {
                ...
                const isFaseGrupos = !!g.grupo;

                // Meta: grupo e data/hora
                const grupoBadge = g.grupo
                    ? `<span style="background:#e3f2fd;color:#1565c0;padding:1px 8px;border-radius:10px;font-size:0.78rem;font-weight:500">${g.grupo}</span>`
                    : "";
                const dataLabel = g.datetime
                    ? `<span style="color:#888;font-size:0.8rem">📅 ${formatDatetime(g.datetime)}</span>`
                    : "";

                if (isFaseGrupos) {
                    // ── Layout INLINE (fase de grupos) ────────────────────────
                    html += `
                    <div style="margin:8px 0;padding:10px 12px;background:white;border-radius:8px;border:1px solid #c8e6c9">
                        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;flex-wrap:wrap">
                            ${grupoBadge}${dataLabel}
                        </div>
                        <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
                            <span style="font-weight:500;flex:1">${flag(g.a)}${g.a}</span>
                            <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
                                <input id="pa_${item.id}" value="${va}" placeholder="0" style="width:48px;text-align:center;padding:7px;font-size:1.05rem">
                                <span style="font-weight:bold;color:#555">×</span>
                                <input id="pb_${item.id}" value="${vb}" placeholder="0" style="width:48px;text-align:center;padding:7px;font-size:1.05rem">
                            </div>
                            <span style="font-weight:500;flex:1;text-align:right">${g.b}${flag(g.b) ? " " + flag(g.b).trim() : ""}</span>
                        </div>
                    </div>`;
                } else {
                    // ── Layout EM DESTAQUE (eliminatórias) ────────────────────
                    html += `
                    <div style="margin:12px 0;padding:14px;background:white;border-radius:10px;border:1px solid #c8e6c9;text-align:center">
                        ${dataLabel ? `<div style="margin-bottom:10px">${dataLabel}</div>` : ""}
                        <div style="display:flex;align-items:center;justify-content:center;gap:20px">
                            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px">
                                <span style="font-size:2rem;line-height:1">${flag(g.a) || "🏳️"}</span>
                                <span style="font-weight:500;font-size:0.9rem">${g.a}</span>
                            </div>
                            <div style="display:flex;align-items:center;gap:8px">
                                <input id="pa_${item.id}" value="${va}" placeholder="0" style="width:52px;text-align:center;padding:8px;font-size:1.15rem">
                                <span style="font-weight:bold;color:#555;font-size:1.1rem">×</span>
                                <input id="pb_${item.id}" value="${vb}" placeholder="0" style="width:52px;text-align:center;padding:8px;font-size:1.15rem">
                            </div>
                            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px">
                                <span style="font-size:2rem;line-height:1">${flag(g.b) || "🏳️"}</span>
                                <span style="font-weight:500;font-size:0.9rem">${g.b}</span>
                            </div>
                        </div>
                    </div>`;
                }
        ...
        function htmlJogoTabela(item) {
            ...
            const isFaseGrupos = !!g.grupo;
            const grupoBadge = g.grupo
                ? `<span style="background:#e3f2fd;color:#1565c0;padding:1px 8px;border-radius:10px;font-size:0.78rem">${g.grupo}</span> `
                : "";
            const dataLabel = g.datetime
                ? `<span style="color:#888;font-size:0.78rem">📅 ${formatDatetime(g.datetime)}</span>`
                : "";

            const cabecalho = isFaseGrupos
                ? `<div style="margin-bottom:4px">${grupoBadge}${dataLabel}</div>
                   <strong>${flag(g.a)}${g.a} × ${g.b}${flag(g.b) ? " " + flag(g.b).trim() : ""}</strong>`
                : `<div style="text-align:center;margin-bottom:8px">
                       ${dataLabel ? `<div style="margin-bottom:8px">${dataLabel}</div>` : ""}
                       <div style="display:flex;justify-content:center;align-items:center;gap:16px">
                           <div style="display:flex;flex-direction:column;align-items:center;gap:3px">
                               <span style="font-size:1.7rem">${flag(g.a) || "🏳️"}</span>
                               <span style="font-size:0.85rem;font-weight:500">${g.a}</span>
                           </div>
                           <span style="color:#aaa">×</span>
                           <div style="display:flex;flex-direction:column;align-items:center;gap:3px">
                               <span style="font-size:1.7rem">${flag(g.b) || "🏳️"}</span>
                               <span style="font-size:0.85rem;font-weight:500">${g.b}</span>
                           </div>
                       </div>
                   </div>`;
        ...
        function htmlRodadaFechada(rodada) {
            ...
            grupos[rodada].forEach(item => {
                ...
                if (isFaseGrupos) {
                    ...
                    <div class="card">
                        <div style="margin-bottom:4px">${grupoBadge}${dataLabel}</div>
                        <strong>${flag(g.a)}${g.a} × ${g.b}${flag(g.b) ? " " + flag(g.b).trim() : ""}</strong><br>
                        Seu palpite: ${guess ? `${guess.a} × ${guess.b}` : "—"}
                    </div>`;
                } else {
                    jogosHtml += `
