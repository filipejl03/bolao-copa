...
// ── ORDENAÇÃO CRONOLÓGICA (mata-mata individual) ───────────────────────────
// Na aba Palpite, jogos do mata-mata (oitavas+) são exibidos em ordem de data/
// hora real (não pela ordem do chaveamento), pois jogos posteriores no
// chaveamento podem ocorrer antes de jogos anteriores na agenda da FIFA.
...
function renderParticipant() {
    ...
    for (const rodada in grupos) {
        ...
        if (rnd.eliminatorio && (rnd.faseIndex || 0) >= 0) {
            const jogos = grupos[rodada];
...
// Para cada fase normal (16 avos até Semifinal) renderiza a coluna da fase
// atual + uma prévia (parcialmente cortada, com opacidade reduzida) da
// próxima fase, conectadas por linhas em ângulo que saem do centro de cada
// par de jogos de origem e convergem no centro do confronto seguinte.
// A fase "Final" é tratada de forma especial: mostra a taça + Grande Final
// + Disputa de 3º lugar (ambas vêm da Semifinal), sem coluna de prévia.
function htmlAbaMataMata(grupos, rounds, allGuesses, participants, uid) {
    ...
    FASES_MM.forEach(fase => {
        ...
        if (fase === "Disputa 3º lugar") return; // exibida junto da Final

        fasesHtml += `<div class="fase-mm-content" id="fase_${fase.replace(/\s+/g,'_')}" style="display:none">
            ${htmlConteudoFaseMM(fase, grupos, rounds)}
        </div>`;
    });
...
// Próxima fase de cada fase normal, para fins de prévia visual
const PROXIMA_FASE_MM = {
    "16 avos de final": "Oitavas de final",
    "Oitavas de final": "Quartas de final",
    "Quartas de final": "Semifinal",
    "Semifinal": "Final"
};

function htmlConteudoFaseMM(fase, grupos, rounds) {
    if (fase === "Final") {
        const jogosFinal    = (grupos["Final"] || []).slice().sort((a, b) => (a.game.ordemMM || 0) - (b.game.ordemMM || 0));
        const jogosTerceiro = (grupos["Disputa 3º lugar"] || []).slice().sort((a, b) => (a.game.ordemMM || 0) - (b.game.ordemMM || 0));

        let html = `<div style="text-align:center;padding:10px 0 6px">
        ...
        jogosFinal.forEach(item => { html += htmlCaixinhaMM(item, true); });

        if (jogosTerceiro.length > 0) {
            html += `<div style="font-size:0.78rem;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.06em;margin:14px 0 6px">Disputa de 3º lugar</div>`;
            jogosTerceiro.forEach(item => { html += htmlCaixinhaMM(item, false); });
        }
        return html;
    }

    const jogos = (grupos[fase] || []).slice().sort((a, b) => (a.game.ordemMM || 0) - (b.game.ordemMM || 0));
    const proxNome = PROXIMA_FASE_MM[fase];
    let proxJogos = proxNome ? (grupos[proxNome] || []).slice().sort((a, b) => (a.game.ordemMM || 0) - (b.game.ordemMM || 0)) : [];

    // Caso especial: a próxima fase da Semifinal é a Final, mas a prévia deve
    // mostrar apenas o jogo da Final (não a Disputa 3º lugar)
    const idColl = "mm_col_" + fase.replace(/\s+/g, '_');

    let jogosHtml = "";
    jogos.forEach((item, i) => { jogosHtml += htmlCaixinhaMM(item, false, idColl + "_a_" + i); });

    let proxHtml = "";
    if (proxJogos.length > 0) {
        proxJogos.forEach((item, i) => { proxHtml += htmlCaixinhaMM(item, false, idColl + "_p_" + i, true); });
    }

    return `
        <div class="mm-fase-wrap" id="${idColl}_wrap">
            <svg class="mm-svg-linhas" id="${idColl}_svg"></svg>
            <div class="mm-fase-cols">
                <div class="mm-col-atual" id="${idColl}_a">${jogosHtml}</div>
                <div class="mm-col-prox" id="${idColl}_p" style="${proxJogos.length === 0 ? 'visibility:hidden' : ''}">${proxHtml}</div>
            </div>
        </div>`;
}

function htmlCaixinhaMM(item, isFinal, cardId, isPreview) {
    const g = item.game;
    ...
    const classes = "mm-card" + (isFinal ? " mm-card-final" : "") + (isPreview ? " mm-card-preview" : "");
...
window.mostrarFaseMM = (fase) => {
    ...
    requestAnimationFrame(() => desenharLinhasMM(fase));
};

// ── LINHAS DE CONEXÃO DO CHAVEAMENTO ───────────────────────────────────────
// Posiciona cada card da coluna de prévia exatamente no centro vertical entre
// os dois jogos de origem correspondentes, e desenha as linhas em ângulo que
// os conectam — reproduzindo visualmente o avanço dos vencedores.
function desenharLinhasMM(fase) {
    const idColl = "mm_col_" + fase.replace(/\s+/g, '_');
    const wrap   = document.getElementById(idColl + "_wrap");
    const svg    = document.getElementById(idColl + "_svg");
    const colA   = document.getElementById(idColl + "_a");
    const colP   = document.getElementById(idColl + "_p");
    if (!wrap || !svg || !colA || !colP) return;

    svg.innerHTML = "";
    const cardsA = Array.from(colA.children);
    const cardsP = Array.from(colP.children);

    if (cardsP.length === 0 || cardsA.length === 0) {
        const h = colA.scrollHeight || 10;
        svg.setAttribute("viewBox", `0 0 380 ${h}`);
        svg.style.height = h + "px";
        wrap.style.height = h + "px";
        return;
    }

    let maxBottom = colA.scrollHeight;
    const x1   = colA.offsetWidth;
    const xMid = x1 + 14;
    const x2   = colP.offsetLeft - colA.offsetLeft;

    function linha(yA, yB, yCentro) {
        const path = `M ${x1} ${yA} L ${xMid} ${yA} L ${xMid} ${yCentro} M ${x1} ${yB} L ${xMid} ${yB} L ${xMid} ${yCentro} M ${xMid} ${yCentro} L ${x2} ${yCentro}`;
    ...
    cardsP.forEach((cardP, i) => {
        const cardA = cardsA[i * 2];
        const cardB = cardsA[i * 2 + 1];
        if (!cardA) return;
        const yA = cardA.offsetTop + cardA.offsetHeight / 2;
        const yB = cardB ? (cardB.offsetTop + cardB.offsetHeight / 2) : yA;
        const yCentro = (yA + yB) / 2;
        const proxTop = yCentro - cardP.offsetHeight / 2;
        cardP.style.top = Math.max(0, proxTop) + "px";
        linha(yA, yB, yCentro);
        maxBottom = Math.max(maxBottom, proxTop + cardP.offsetHeight);
    });

    const totalH = maxBottom + 10;
    svg.setAttribute("viewBox", `0 0 380 ${Math.ceil(totalH)}`);
...
function htmlRodadaAberta(rodada, grupos, myGuesses, allGuesses, participants, uid, rounds) {
    const rnd = rounds[rodada] || {};
    const isElimIndiv = rnd.eliminatorio && (rnd.faseIndex || 0) >= 0;
