...
function loadParticipant(uid) {
    Promise.all([
        ...
        for (const rodada in grupos) {
            ...
            h += `<h3>${rodada} — ${open ? "🟢 Aberta para palpites" : "🔴 Fechada"}</h3>`;

            if (open) {
                // ── Rodada aberta: participante faz/edita seus palpites
                // Coleta IDs dos jogos da rodada para o botão salvar todos
                ...
                grupos[rodada].forEach(item => {
                    ...
                    h += `
                        <div class="card">
                            ...
                            Palpite:
                            <input id="pa_${item.id}" size="2" value="${va}" placeholder="A">
                            x
                            <input id="pb_${item.id}" size="2" value="${vb}" placeholder="B">
                        </div>`;
                });

                h += `<button onclick="saveAllGuesses('${uid}','${idsRodada}')" style="width:100%;padding:12px;font-size:1rem;margin-top:4px">
                    💾 Salvar todos os palpites de ${rodada}
                </button>`;

            } else {
                // ── Rodada fechada: mostra palpites de todos + resultado + pontos
                grupos[rodada].forEach(item => {
                    const g       = item.game;
                    ...
                    Object.keys(participants).forEach(pid => {
                        ...
                        if (!guess) {
                            linhas += `<tr><td>${participants[pid]}</td><td>—</td><td>—</td><td>—</td></tr>`;
                            return;
                        }
                        let pontosTd = "—";
                        if (temResult) {
