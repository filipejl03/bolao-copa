...
function loadParticipant(uid) {
    Promise.all([
        ...
        // ── Rodada EM ANDAMENTO: sem campos de palpite, pontuação parcial (visível) ─
        function htmlRodadaAndamento(rodada) {
            ...
            grupos[rodada].forEach(item => {
                ...
                let linha = `<strong>${g.a} x ${g.b}</strong><br>`;

                if (guess) {
                    linha += `Seu palpite: ${guess.a} x ${guess.b}`;
                    if (temResult) {
                        const r = calcPontos(guess.a, guess.b, g.resultA, g.resultB);
                        linha += ` &nbsp;—&nbsp; Resultado: <strong>${g.resultA} x ${g.resultB}</strong> &nbsp;—&nbsp; `;
                        linha += r
                            ? `<span style="font-weight:bold;color:#2980b9">${r.cod} +${r.pts}pts</span>`
                            : `<span style="color:#c0392b">0 pts</span>`;
                    } else {
                        linha += ` &nbsp;—&nbsp; <em>aguardando resultado</em>`;
                    }
                } else {
                    linha += temResult
                        ? `Você não palpitou &nbsp;—&nbsp; Resultado: <strong>${g.resultA} x ${g.resultB}</strong>`
                        : `Você não palpitou &nbsp;—&nbsp; <em>aguardando resultado</em>`;
                }

                html += `
                    <div style="margin:8px 0;padding:8px;background:white;border-radius:6px;border:1px solid #ffe082">
                        ${linha}
                    </div>`;
            });
        ...
        function htmlRodadaEncerrada(rodada) {
            ...
            grupos[rodada].forEach(item => {
                const g = item.game;
                const resultadoLabel = `<span style="color:#27ae60"><strong>Resultado: ${g.resultA} x ${g.resultB}</strong></span>`;
