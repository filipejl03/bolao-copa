...
function htmlAbaDesempenho(grupos, allGuesses, participants, uid, gamesSnap, ranking) {
    ...
    Object.keys(jogosPorRodada).forEach(rodada => {
        ...
        jogos.forEach(({ id, game: g }) => {
            // Trava: outro participante só visível em andamento ou encerrado
            ...
            const statusJogo = isElimIndiv ? getGameStatus(g) : getRoundStatus(rodada, rounds, jogosPorRodada);
            const palpiteVisivel = !isOutroParticipante || statusJogo === "andamento" || statusJogo === "encerrada";
